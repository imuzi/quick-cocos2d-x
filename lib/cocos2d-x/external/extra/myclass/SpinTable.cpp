...
void SpinTable::spin()
{
    ...
    for (int i=0; i<m_iColumnNum; i++) {
        ...
        for (int j=0; j<m_iRowNum+2; j++) {
            SpinTableCell * cell = (SpinTableCell *)columnNodes->objectAtIndex(j);
            cell->runAction(CCSequence::create(CCEaseBackIn::create(CCMoveBy::create(0.75, ccp(0, m_szCellFrame.height*3/2))), CCCallFunc::create(this, callfunc_selector(SpinTable::startSpin)), NULL));
        }
    }
}
