...
//    disableTouch = false;
    doTouchBeganWhenMovedFistTime = true;
...
float EXZoomController::getOptimalZoomOutLimit() {
    ...
    printf("(_winTr.x - _winBl.x),,%f",(_winTr.x - _winBl.x));
    printf("(_winTr.y - _winBl.y),,%f",(_winTr.y - _winBl.y));
...
//    float adjustWidth=_node->boundingBox().size.width*_node->getAnchorPoint().x-_node->getPositionX();
//    float adjustHeight=_node->boundingBox().size.height*_node->getAnchorPoint().y-_node->getPositionY();
//    
//    CCPoint newPos = pos;
//    CCSize  winSize = CCDirector::sharedDirector()->getWinSize();
//    newPos.x=  MIN(newPos.x, adjustWidth);
//    newPos.x = MAX(newPos.x, winSize.width-_node->boundingBox().size.width+adjustWidth);
//    newPos.y = MIN(newPos.y,adjustHeight);
//    newPos.y = MAX(newPos.y, winSize.height-_node->boundingBox().size.height+adjustHeight);
//    //Correct for anchor
...
//        CCLog("touc id %s,",CCString::createWithFormat("%d",pTouch->getID())->getCString());
	...
	multitouchnum = _touchesDic->count();
	bool  multitouch = multitouchnum > 1;
//	CCLog("CCTOUCHBeGAN"); 
	if (multitouch){
...
//    }
//    multitouchnum = _touchesDic->count();
    if (doTouchBeganWhenMovedFistTime){
        ccTouchesBegan( pTouches,  pEvent) ;
        doTouchBeganWhenMovedFistTime = false;
...
//        return;
//    }
//    CCLog("multitochnum,%d",multitouchnum); 
	bool  multitouch = _touchesDic->count() > 1;
	if (multitouch) {
        CCArray* keys = _touchesDic->allKeys();
        CCTouch *touch1 = (CCTouch*)_touchesDic->objectForKey(((CCString*)keys->objectAtIndex(0))->getCString());
		...
		endZoom(pt1, pt2);
//    CCLog("ccTouchesEndedccTouchesEnded2222"); 
		//which touch remains?
		if (touch2)
		...
		endScroll(pt);
//        CCLog("ccTouchesEndedccTouchesEnded1111");
...
//            handleDoubleTapAt(pt);
    ...
    CCLog("_node is LandMap.layer,posx=%f,posy=%f,scale = %f",_node->getPositionX(),_node->getPositionY(),_node->getScale());
...
void EXZoomController::endScroll(CCPoint pos) {
	...
	if (_timePointStampCounter > 3) {
		...
		if (ccpLengthSQ(velocity) > 5*5) {
			...
			CCEaseOut* ease = CCEaseOut::create(moveTo, 2.5);
...
void EXZoomController::moveZoom(CCPoint pt, CCPoint pt2) {
    ...
    if (centerOnPinch)
        centerOnPoint(_node->convertToNodeSpace(ccpMidpoint(pt, pt2)), factor);
    else
...
void EXZoomController::centerOnPoint(CCPoint pt, float damping) {
    ...
    CCPoint diff = ccpMult(ccpSub(pt, mid), 0.04);
    updatePosition(ccpAdd(_node->getPosition(), diff));
//// add lxc
//    printf("damping, %f",damping);
//    CCPoint prevAnchor=_node->getAnchorPoint();
//    CCPoint size = ccpSub(_tr, _bl);
//    float ww = size.x/2;
//    float hh = size.y/2;
//    CCPoint apoint = ccp(pt.x/ww,pt.y/hh);
//    _node->setAnchorPoint(apoint);
//    printf("_node->boundingBox().size.width,apoint,%f,%f",_node->boundingBox().size.width,size.x);
//    printf("_node->boundingBox().heigh,apoint,%f,%f",_node->boundingBox().size.height,size.y);
   
    //调整后的地图位置
//    CCPoint newPos =ccp(ww*apoint.x*_node->getScale()+_node->getPositionX()-_node->boundingBox().size.width*prevAnchor.x,hh*apoint.y*_node->getScale()+_node->getPositionY() -_node->boundingBox().size.height*prevAnchor.y);
